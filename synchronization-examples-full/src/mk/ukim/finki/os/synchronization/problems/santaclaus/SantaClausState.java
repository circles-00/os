...
public class SantaClausState extends AbstractState {
			...
			if (!firstHitched) {
				...
				if (arrivedReindeers != 9) {
					PointsException e = new PointsException(
							ALL_RAINDEERS_MUST_ARRIVE_POINTS,
							ALL_RAINDEERS_MUST_ARRIVE);
					log(e, null);
				} else if (!sleighReady) {
					PointsException e = new PointsException(
							SLEIGH_NOT_READY_POINTS, SLEIGH_NOT_READY);
					log(e, null);
				} else {
	...
	public synchronized void prepSleigh() {
		if (arrivedReindeers != 9) {
			PointsException e = new PointsException(
					REINDEERS_NOT_PRESENT_POINTS, REINDEERS_NOT_PRESENT);
			log(e, null);
		} else {
	...
	public void elfEntered() {
		...
		synchronized (this) {
			if (elvesWantHelp) {
				PointsException e = new PointsException(MAX_3_ELVES_POINTS,
						MAX_3_ElVES);
				log(e, null);
			} else {
	...
	public void getHelp() {
		...
		synchronized (this) {
			if (!elvesWantHelp) {
				PointsException e = new PointsException(
						ALL_ELVES_MUST_ENTER_POINTS, ALL_ELVES_MUST_ENTER);
				log(e, null);
			} else if (!santaHelped) {
				PointsException e = new PointsException(
						SANTA_DID_NOT_HELP_POINTS, SANTA_DID_NOT_HELP);
				log(e, null);
			} else {
	...
	public synchronized void helpElves() {
		if (arrivedReindeers == 9 && !sleighReady && elvesWantHelp) {
			PointsException e = new PointsException(
					MUST_PREPARE_SLEIGH_FIRST_POINTS, MUST_PREPARE_SLEIGH_FIRST);
			log(e, null);
		} else if (!elvesWantHelp) {
			PointsException e = new PointsException(ELVES_NOT_PRESENT_POINTS,
					ELVES_NOT_PRESENT);
			log(e, null);
		} else if (elvesWantHelp) {
